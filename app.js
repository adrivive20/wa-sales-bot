...
const languageStore = new Map();  // userId -> {lang, streak, lastCandidate} (según tu salesAgent)

// ✅ Estados de handoff
const handoffActive = new Set();          // userId en modo humano: bot no responde
...
async function start() {
  ...
  // ✅ Dedupe por mensaje (evita dobles respuestas)
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    try {
      ...
      // Ignorar mensajes propios y status
      ...
      // Evitar spam por eventos seguidos
      ...
      // Si está en modo humano, el bot no responde (salvo reactivar)
      ...
      if (handoffPendingConfirm.has(userId)) {
        ...
        // Si responde otra cosa, pedimos confirmación de nuevo sin ser necios
        await sock.sendMessage(remoteJid, {
      ...
      // ✅ Si detecta intención de hablar con humano, pedir confirmación
